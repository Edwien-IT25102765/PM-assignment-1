#include <stdio.h>
#include <stdlib.h>
#include <time.h>

#define min 3
#define max 10

char board[max][max];
int n,i,j,k;
int winRow;
int lastRow = -1, lastCol = -1;
FILE *logFile;

// Display board
void displayBoard(){
    system("clear"); // clear the terminal for current game frame

    printf("\nTic Tac Toe (%dx%d) - %d in a row to win\n", n, n, winRow);

    // Column numbers
    printf("   ");
    for (j = 0; j < n; j++) {
        if (j+1 < 10) printf("   %d  ", j+1);
        else printf("  %d   ", j+1);
    }
    printf("\n");

    // Top border
    printf("   ");
    for (j = 0; j < n; j++) printf("------");
    printf("-\n");

    for (i = 0; i < n; i++) {
        // Row number
        if (i+1 < 10) printf(" %d |", i+1);
        else printf("%d |", i+1);

        for (j = 0; j < n; j++) {
            if (i == lastRow && j == lastCol)
                printf(" *%c* |", board[i][j]); // last move highlighted
            else
                printf("  %c  |", board[i][j]);
        }
        printf("\n");

        // Horizontal divider
        printf("   ");
        for (j = 0; j < n; j++) printf("------");
        printf("-\n");
    }

    if(lastRow != -1) printf("Last move by %c is marked with *\n", board[lastRow][lastCol]);
    printf("\n");
}

// Log moves to file
void logBoard(char player){
    fprintf(logFile, "Player %c made a move:\n", player);
    for (i = 0; i < n; i++){
        for (j = 0; j < n; j++) fprintf(logFile, "%c ", board[i][j]);
        fprintf(logFile, "\n");
    }
    fprintf(logFile, "\n");
}

// Check if it's a draw
int checkDraw(){
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
            if (board[i][j]==' ') return 0;
    return 1;
}

// Validate move
int checkValidMove(int row, int col){
    return (row >= 0 && row < n && col >= 0 && col < n && board[row][col] == ' ');
}

// Human player move
void playerMove(char player){
    int row, col;
    while (1) {
        printf("Player %c, where would you like to place your mark? (row col, 1-%d): ", player, n);
        if (scanf("%d %d", &row, &col) != 2){
            while (getchar() != '\n'); // clear invalid input
            printf("Oops! That spot is taken or invalid. Please try again.\n");
            continue;
        }
        row--; col--;
        if (checkValidMove(row, col)) {
            board[row][col] = player;
            lastRow = row;
            lastCol = col;
            break;
        } else {
            printf("Oops! That spot is taken or invalid. Please try again.\n");
        }
    }
}

// Computer move (random)
void computerMove(){
    int row, col;
    do {
        row = rand() % n;
        col = rand() % n;
    } while (!checkValidMove(row, col));

    board[row][col] = 'O';
    lastRow = row;
    lastCol = col;
    printf("\nComputer placed O at (%d, %d)\n", row+1, col+1);
}

// Check winner
int checkWinner(){
    // Rows
    for (i=0;i<n;i++)
        for (j=0;j<=n-winRow;j++){
            char first = board[i][j];
            if (first==' ') continue;
            int win = 1;
            for (k=1;k<winRow;k++)
                if (board[i][j+k] != first){win=0; break;}
            if (win) return first;
        }

    // Columns
    for (j=0;j<n;j++)
        for (i=0;i<=n-winRow;i++){
            char first = board[i][j];
            if (first==' ') continue;
            int win = 1;
            for (k=1;k<winRow;k++)
                if (board[i+k][j] != first){win=0; break;}
            if (win) return first;
        }

    // Diagonals
    for (i=0;i<=n-winRow;i++)
        for (j=0;j<=n-winRow;j++){
            char first = board[i][j];
            if (first==' ') continue;
            int win = 1;
            for (k=1;k<winRow;k++)
                if (board[i+k][j+k] != first){win=0; break;}
            if (win) return first;
        }

    // Anti-diagonals
    for (i=0;i<=n-winRow;i++)
        for (j=winRow-1;j<n;j++){
            char first = board[i][j];
            if (first==' ') continue;
            int win = 1;
            for (k=1;k<winRow;k++)
                if (board[i+k][j-k] != first){win=0; break;}
            if (win) return first;
        }

    return 0;
}

// Setup game
void setupGame(){
    for (i=0;i<n;i++)
        for (j=0;j<n;j++)
            board[i][j] = ' ';

    logFile = fopen("game_log.txt", "w"); // overwrite previous logs
    if (!logFile){
        printf("Error: Cannot open log file.\n");
        exit(1);
    }

    // Decide winRow based on board size
    if (n <= 4) winRow = 3;
    else if (n <= 7) winRow = 4;
    else winRow = 5;
}

// Close game
void closeGame(){
    fclose(logFile);
}

// Main control
void playGame(){
    char current = 'X';
    int winner = 0;

    while (!winner && !checkDraw()){
        displayBoard();
        if (current == 'X') playerMove(current);
        else computerMove();

        logBoard(current);
        winner = checkWinner();
        if (!winner) current = (current == 'X') ? 'O' : 'X';
    }

    displayBoard();
    printf("\n🎮 Game Over! 🎮\n");
    if (winner) printf("🎉 Player %c wins! Congratulations! 🎉\n", winner);
    else printf("🤝 It's a draw! Well played both!\n");
    printf("Thanks for playing Tic Tac Toe!\n");
}

// Main function
int main(void){
    srand(time(NULL)); // seed random number generator
    printf("--- Tic Tac Toe (User vs Computer) ---\n");
    printf("Enter board size (%d–%d): ", min, max);
    scanf("%d", &n);
    if (n < min || n > max){
        printf("Invalid board size. Exiting game.\n");
        return 1;
    }

    setupGame();
    playGame();
    closeGame();

    return 0;
}

